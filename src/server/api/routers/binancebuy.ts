...
export const BinanceBuyOrderRouter = createTRPCRouter({
  intiateBinanceBuyOrder: publicProcedure
    .input(
      z.object({
      ...
      if (
        ...
        if (mpesaAccessToken) {
          ...
          const stkPush = await sendStkPush({
            businessShortCode: process.env.MPESA_SANDBOX_SHORT_CODE!,
            ...
            partyB: process.env.MPESA_SANDBOX_SHORT_CODE!,
  ...
  checkBinanceBuyOrderSTKpaymentStatus: publicProcedure
    .input(
      z.object({
        orderId: z.string(),
      }),
